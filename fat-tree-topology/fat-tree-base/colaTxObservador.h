...
// Clase que hará de observadora (se subscribirá a trazas de fuente de los elementos de red)
class ColaTxObservador
{
public:
  // Constructor: recibe como parámetros todos los elementos a los que necesita 
  // suscribirse para realizar su función (cola_tx) 
          ColaTxObservador (Ptr<Queue<Packet>> cola_tx); 

  // Método para atender los encolados
  void    EncolaPq (Ptr<const Packet> paquete);

  // Método para atender los desencolados
  void    DesencolaPq (Ptr<const Packet> paquete);

  // Método para atender los paquetes eliminados
  void    EliminaPq (Ptr<const Packet> paquete);
