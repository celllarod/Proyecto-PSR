...
NS_LOG_COMPONENT_DEFINE ("Bcube_prueba1");


// *************************
// ** Declaración métodos **
// *************************
ObservadorPaquetes escenario(uint32_t tamPaquete, Time intervalo, uint32_t maxPaquetes, Time delay_p2p, Time delay_csma, DataRate bitrate_p2p, DataRate bitrate_csma);
...
int main(int argc, char *argv[]){
  ...
  uint32_t maxPaquetes = 57895;
  DataRate bitrate_p2p ("10Gbps");
  DataRate bitrate_csma ("10Mbps");
  ...
  Gnuplot mi_grafica = grafica (tamPaquete, intervalo, maxPaquetes, retardo_canal_p2p, retardo_canal_csma, bitrate_p2p, bitrate_csma);
  std::ofstream fichero("prueba1-bcube.plt");
  mi_grafica.GenerateOutput(fichero);
...
Punto punto (uint32_t tamPaquete, Time intervalo, uint32_t maxPaquetes, Time delay_p2p, Time delay_csma, DataRate bitrate_p2p, DataRate bitrate_csma){
    ...
    for(uint32_t i=0 ; i<ITERACIONES ; i++){
      ObservadorPaquetes obs = escenario(tamPaquete, intervalo, maxPaquetes, delay_p2p, delay_csma, bitrate_p2p, bitrate_csma);
      media.Update(100*obs.GetPerdidos()/obs.GetCuentaTx());
      NS_LOG_DEBUG ("[GRAFICA] \tSIMULACIÓN " << i+1 << " de " << ITERACIONES << " -- PQ_PERDIDOS = " << obs.GetPerdidos() << " -- \% PERDIDOS = " << (100*obs.GetPerdidos()/obs.GetCuentaTx()));   
    }
    err=TSTUDENT*sqrt(media.Var()/media.Count());

    return Punto(bitrate_csma.GetBitRate()/1000000,media.Mean(),err);
}
...
Gnuplot2dDataset curva (uint32_t tamPaquete, Time intervalo, uint32_t maxPaquetes, Time delay_p2p, Time delay_csma, DataRate bitrate_p2p, DataRate bitrate_csma){
    ...
    std::string nombrecurva = " ";
    //nombrecurva.append(std::to_string(num_nodos));
    ...
    DataRate tasa_csma = bitrate_csma;
    for(uint32_t i=0 ; i<NUM_PUNTOS ; i++){
      Punto point = punto(tamPaquete, intervalo, maxPaquetes, delay_p2p, delay_csma, bitrate_p2p, tasa_csma);
      mi_curva.Add(point.abscisa(),point.ordenada(),point.error());
      NS_LOG_DEBUG ("[GRAFICA] \t--> PUNTO " << i+1 << " de " << NUM_PUNTOS << " -- TASA = " << tasa_csma << " (Tasa,PqPerd,error) = (" << point.abscisa() <<", " << point.ordenada() <<", " << point.error() << ")");
...
Gnuplot grafica (uint32_t tamPaquete, Time intervalo, uint32_t maxPaquetes, Time delay_p2p, Time delay_csma, DataRate bitrate_p2p, DataRate bitrate_csma){
    ...
    Gnuplot grafica;
    NS_LOG_DEBUG ("[GRAFICA] ------------------------------------------------------------------------------");
    NS_LOG_DEBUG ("[GRAFICA] CURVA " << 1 << " de " << NUM_CURVAS);
    grafica.SetTitle("Prueba 1 - BCube: Paquetes perdidos (%) vs Tasa canal CSMA (Mbps)");
    grafica.SetLegend("Tasa del canal CSMA (Mbps)","Paquetes perdidos (%)");
