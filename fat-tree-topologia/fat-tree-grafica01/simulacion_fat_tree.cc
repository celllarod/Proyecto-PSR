...
{ 
  ...
  NS_LOG_INFO("----------------------------------------------------------------------------");
  NS_LOG_INFO("[Cliente_1] Cliente 1 conectado al switch 1 del core.");
  ...
  Ptr<NetDevice> dispo = n_servidor1->GetDevice(1);
  ...
  h_csma_pcap.EnablePcap("fuente1-fattree", n_servidor1->GetDevice(1));
  h_csma_pcap.EnablePcap("sumidero1-fattree", n_cliente1->GetDevice(1));
...
int main (int argc, char *argv [])
{
  ...
  std::ofstream fichero ("prueba1-fattree.plt");
      ...
      )
{
    ...
    Punto pto_curva (tasaEnvioCsma.GetBitRate()/1000000, ac_perdidos.Mean(), error);
...
{
  Gnuplot grafica;
  grafica.SetTitle ("Prueba 1  - Fat-Tree: Paquetes perdidos (%) vs Tasa del canal CSMA");
